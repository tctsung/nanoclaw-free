...
function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  // ponytail: free baseline sends the real key straight to the provider so
  // OpenCode's own provider def picks the right auth header (Zen → x-api-key).
  // The host adds the provider host to NO_PROXY so this call skips the OneCLI
  // proxy, whose Bearer injection broke Zen. Falls back to 'placeholder' (proxy
  // path) when no key is set — i.e. Anthropic-via-vault still works unchanged.
  const directKey = process.env.OPENCODE_API_KEY;
  ...
  const providerOptions: Record<string, unknown> =
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            options: {
              apiKey: directKey?.trim() ? directKey : 'placeholder',
              baseURL: proxyUrl,
            },
